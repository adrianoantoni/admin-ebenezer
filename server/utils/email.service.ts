...
export const sendResetPasswordEmail = async (email: string, token: string) => {
    ...
    const mailOptions = {
        from: `"Equipe EclesiaMaster" <${process.env.SMTP_FROM || 'noreply@eclesiamaster.com'}>`,
        to: email,
        subject: 'Recuperação de Senha - EclesiaMaster',
        html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; rounded: 12px;">
        <h2 style="color: #1e3a8a; text-align: center;">EclesiaMaster</h2>
        <p>Olá,</p>
        <p>Recebemos uma solicitação para redefinir a senha da sua conta no EclesiaMaster.</p>
